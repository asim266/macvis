...
export const MCP_REGISTRY: MCPServerDef[] = [
  ...
  // ─── BROWSER / AUTOMATION ───────────────────────────────────────────────────
  {
    id: 'playwright',
    name: 'Playwright',
    description: 'Drive a real browser: navigate, click, fill, scrape',
    category: 'search',
    source: 'vendor',
    featured: true,
    brandColor: 'oklch(64% 0.17 150)',
    icon: '🎭',
    command: 'npx',
    args: ['-y', '@playwright/mcp@latest'],
    setupHint: 'No key needed. First run downloads a browser engine.',
  ...
  // ─── MORE DEPLOYMENT / PRODUCTIVITY ─────────────────────────────────────────
  {
    id: 'gitlab',
    name: 'GitLab',
    description: 'Repos, MRs, issues, pipelines',
    category: 'deployment',
    source: 'official',
    icon: '🦊',
    command: 'npx',
    args: ['-y', '@modelcontextprotocol/server-gitlab'],
    env: { GITLAB_PERSONAL_ACCESS_TOKEN: 'mcps.gitlab.token', GITLAB_API_URL: 'mcps.gitlab.apiUrl' },
    inputs: [
      { label: 'Personal Access Token', configKey: 'mcps.gitlab.token', type: 'password', placeholder: 'glpat-...' },
      { label: 'API URL (optional)', configKey: 'mcps.gitlab.apiUrl', type: 'text', placeholder: 'https://gitlab.com/api/v4' },
    ],
  ...
  },
  {
    id: 'sentry',
    name: 'Sentry',
    description: 'Errors, issues, and performance from your apps',
    category: 'deployment',
    source: 'vendor',
    icon: '🛡',
    command: 'npx',
    args: ['-y', '@sentry/mcp-server'],
    env: { SENTRY_AUTH_TOKEN: 'mcps.sentry.token' },
    inputs: [{ label: 'Auth Token', configKey: 'mcps.sentry.token', type: 'password' }],
    docsUrl: 'https://sentry.io/settings/account/api/auth-tokens/',
  },
  {
    id: 'figma',
    name: 'Figma',
    description: 'Read designs, frames, and styles for hand-off to code',
    category: 'productivity',
    source: 'community',
    icon: '🎨',
    command: 'npx',
    args: ['-y', 'figma-developer-mcp', '--stdio'],
    env: { FIGMA_API_KEY: 'mcps.figma.apiKey' },
    inputs: [{ label: 'API Key', configKey: 'mcps.figma.apiKey', type: 'password' }],
    docsUrl: 'https://www.figma.com/developers/api#access-tokens',
  },
  {
    id: 'airtable',
    name: 'Airtable',
    description: 'Read/write bases, tables, and records',
    category: 'database',
    source: 'community',
    icon: '🗃',
    command: 'npx',
    args: ['-y', 'airtable-mcp-server'],
    env: { AIRTABLE_API_KEY: 'mcps.airtable.apiKey' },
    inputs: [{ label: 'Personal Access Token', configKey: 'mcps.airtable.apiKey', type: 'password', placeholder: 'pat...' }],
  ...
  },
  {
    ...
    env: { GOOGLE_MAPS_API_KEY: 'apiKeys.googleMaps' },
    inputs: [{ label: 'API Key', configKey: 'apiKeys.googleMaps', type: 'password' }],
